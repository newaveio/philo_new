...
int find_philo_to_eat(t_data *data)
{
    int i;
    int philo_to_eat;
    long min_last_meal;
    ...
    {
        if (get_int(&data->philos[i].meal_check, &data->philos[i].meals) == get_int(&data->min_meals_mut, &data->min_meals) && \
            data->philos[i].last_meal < min_last_meal)
        {
